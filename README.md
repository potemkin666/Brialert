...
[![Feed validation](https://github.com/potemkin666/AlbertAlert/actions/workflows/ci-feed-validation.yml/badge.svg)](https://github.com/potemkin666/AlbertAlert/actions/workflows/ci-feed-validation.yml)
[![Update live feeds](https://github.com/potemkin666/AlbertAlert/actions/workflows/update-live-feed.yml/badge.svg)](https://github.com/potemkin666/AlbertAlert/actions/workflows/update-live-feed.yml)

AlbertAlert is a GitHub Pages web app for fast terrorism monitoring (UK/EU bias)

## Live site

[https://potemkin666.github.io/AlbertAlert/](https://potemkin666.github.io/AlbertAlert/)
...
- Backend env vars required for quarantine admin auth:
  - `ALBERTALERT_SESSION_SECRET` (strong random secret for signing session/state cookies)
  - optional: `ALBERTALERT_SESSION_TTL_SECONDS` (defaults to `28800`, i.e. 8h), `ALBERTALERT_OAUTH_STATE_TTL_SECONDS` (defaults to `600`, i.e. 10m)
...
- Feed validation CI runs source health in a critical-only scope (`ALBERTALERT_SOURCE_HEALTH_SCOPE=critical`) with a curated high-value London source list (`ALBERTALERT_SOURCE_HEALTH_CRITICAL_IDS`) and runs feed build in a bounded smoke configuration; scheduled hourly refresh keeps full-depth source checks.
...
- Build/runtime knobs (timeouts, retries, prefetch counts, html budget, guardrail fail behavior) are configurable through `ALBERTALERT_*` environment variables for CI fast-mode tuning.
